...
do -- swap args
  ...
  remap('n', '<leader>a', function() swap.swap_next '@parameter.inner' end, { desc = 'Swap next inner [a]rgument' })
  remap('n', '<leader>A', function() swap.swap_previous '@parameter.inner' end, { desc = 'Swap previous inner [A]rgument' })
end

do -- Repeatable commands
...
local function next_qflist()
  if vim.fn.getqflist({ winid = 0 }).winid > 0 then
    ...
    if not success then vim.notify('Reached end of list', vim.log.levels.ERROR) end
  end
end
...
local function prev_qflist()
  if vim.fn.getqflist({ winid = 0 }).winid > 0 then
    ...
    if not success then vim.notify('Reached beginning of list', vim.log.levels.ERROR) end
  end
end
...
local function prev_loclist()
  if vim.fn.getloclist(1, { winid = 0 }).winid > 0 then
